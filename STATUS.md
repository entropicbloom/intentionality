...
1. Frame-source question (decides the v2 mechanism section, currently written as "residual anisotropy fixes the frame"): `abl_resid_is` = 20.7° raw / 20.7° aligned with the 8-class residual removed (unablated 20.2°) -> the 8-class anisotropy is NOT the anchor. Pending: `abl_resid_iv`, `abl_circ_{is,iv}` (control: whole class structure removed, expect structure loss), `abl_resid24_{is,iv}` (24-class), `bal_is` (class-balanced populations, n/8 per bin), `bal_abl_is`, `bal_abl24_{is,iv}`. Read: if `bal_*` raw error rises toward 45° while aligned stays ~20°, label density (cardinal overrepresentation, V1 peaks at 90°, AL at 0°) is the anchor -> rewrite Section 3.2 paragraph 2 and the discussion accordingly ("what breaks the symmetry is the uneven occupancy of the orientation circle"), keep the rotated-label and cross-area results (they are consistent with density too), drop the claim that the correlation residual supplies the frame. If bal_* keeps the frame and bal_abl24 loses it, both contribute. If nothing removes the frame, say the anchor is unidentified and list what was excluded.
...
3. When all three markers are present: PULL FIRST (`decoder2.json`, `allen/outputs/decoder.json`, `synthetic.json`, `preds/*.npz`, `logs/`; use a quoted ssh options string or `${=VAR}` in zsh), merge (dict update), verify tag counts locally, THEN `podTerminate hr5n7tufcpag6c` in a separate command, check `clientBalance` (was $11.25).
...
| residual ablation (v2) | `abl_{resid,circ}_{is,iv}` (`ablate=resid|circ`, 17M) | subtract the class-Gram residual (or the whole class structure) from every Gram using true classes: does the frame collapse while the aligned error survives | **twin: 20.7° raw / 20.7° aligned with the 8-class residual removed (unablated 20.2°): the frame survives, so the frame source is NOT the 8-class mean anisotropy.** Follow-ups queued as stream c: `abl_resid24_{is,iv}` (24-class residual), `bal_is` (class-balanced populations, n/8 per bin), `bal_abl_is`, `bal_abl24_{is,iv}` (balanced + ablation) to separate finer-resolution anisotropy from label density. in vivo + circ controls running |
