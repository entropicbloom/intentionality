...
| residual ablation (v2) | `abl_{resid,circ}_{is,iv}` (`ablate=resid|circ`, 17M) | subtract the class-Gram residual (or the whole class structure) from every Gram using true classes: does the frame collapse while the aligned error survives | queued (pod 6) |
