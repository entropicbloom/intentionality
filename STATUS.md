...
| class-balanced populations (v2) | `bal_is`, `bal_iv`, `bal_is_s1`, `bal_iv_s1` (`balance_pop=1`: 64 neurons per 8-class bin, train and test; labels only select neurons) | is the absolute frame the label density | **twin 22.4° raw / 22.3° aligned, in vivo 26.5° / 26.4° (best epoch 9 / 5): the frame survives class balancing, and beats the standard decoder's balanced error (24.5 / 27.9). Label density is NOT the anchor.** Seeds 1 running (stream e) |
