...
### Resume checklist (written 2026-09-13 ~16:00 UTC before a context compaction)
...
| extra seeds (v2) | `a_*_ori_s{1,2}`, `lin_*_s{1,2}`, `bp_*_ori_s{1,2}`, `c_*_17M_bins_s{1,2}`, `c_is_17M_statsbias_s{1,2}` | spreads for the reference and controls tables | queued (pod 6) |
| synthetic v2 | `syn5_c*s*t*_d{0..4}` (5 seeds, selection on raw error), `syn5_sharp{4,8,16,32}_d{0,1}`, `syn5_gain{0.5,1,2,4}_d{0,1}` | the block the reviewer asked for, plus strength sweeps of neuron-side anisotropy with an isotropic stimulus | queued (pod 6) |
