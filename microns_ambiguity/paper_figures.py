...
def fig_pipeline():
    ...
    save(fig, "fig1_microns_pipeline", "How the MICrONS task is built",
         ...
         "components); shown are six real neurons and their correlation matrix. The oracle clips were shown in all 13 scans, so the matrix is defined across scans (same-scan pairs correlate ~1.5x more than cross-scan pairs at matched RF distance in vivo, ~1.3x on the twin). The decoder only ever sees such correlations, never the stimulus. (c) The contents "
