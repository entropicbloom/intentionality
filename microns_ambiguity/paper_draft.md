...
The question is worth asking for two reasons. In practice, correlations are often all that is available: recordings under natural behaviour have no controlled stimulus, and neurons recorded in different sessions or different animals share no stimulus-aligned coordinate system, but they do share the structure of their correlations. In principle, the answer says how much of a neuron's representational content is fixed by its position in the network of relations among neurons, which is the question raised in Lässig (2025).
