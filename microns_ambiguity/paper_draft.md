...
The question has two parts. The first is whether the information is present: does the correlation matrix over a population determine, up to the symmetries of the content, which neuron prefers which orientation and where its receptive field lies. The second is whether it is present in a form that generalises: whether a decoder trained on the relations of some neurons reads the relations of other neurons, and of other brains.
