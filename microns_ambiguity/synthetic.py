...
def make(n=6000, T=120, count=0, sharp=0, stim=0, noise=0.5, seed=0, amp_var=0):
    rng = np.random.default_rng(seed)
    pref = vonmises_pref(n, rng, count)
    kappa = np.full(n, 2.0)
    if sharp: kappa = 2.0 + 2.0 * np.cos(np.deg2rad(2 * pref)) ** 2       # 4 at 0°/90°, 2 at 45°/135°
    ...
    R = amp[None, :] * np.exp(kappa[:, None] * (np.cos(d) - 1)) + noise * rng.normal(size=(n, T))
    return R.astype(np.float32), pref.astype(np.float32)


def main(tag, count=0, sharp=0, stim=0, n_neurons=6000, T=120, noise=0.5, data_seed=0, amp_var=0, **kw):
    F, pref = make(n_neurons, T, count, sharp, stim, noise, data_seed, amp_var)
    ...
    m.update(synthetic=dict(count=count, sharp=sharp, stim=stim, n_neurons=n_neurons, T=T, noise=noise, data_seed=data_seed, amp_var=amp_var), seconds=time.time() - t0)
...
if __name__ == "__main__":
    tag = sys.argv[1]; kw = {}
    for a in sys.argv[2:]:
        k, v = a.split("="); kw[k] = v if k in ("device", "input_mode") else (bool(int(v)) if k in ("rel_bias", "row_proj", "label_rot", "ori_weight") else (float(v) if k in ("lr", "early_stop", "dropout", "noise") else int(v)))
    main(tag, **kw)
