...
- **Mechanism (symmetry).** The class-Gram is near-circulant (71 % / 81 % of variance), which fixes orientation only up to rotation and reflection; the residual (29 % / 19 % of class-Gram variance) is reproducible across neuron halves (0.94) and disjoint stimulus halves (0.84 / 0.99) and unchanged on class-balanced subsamples. Trained with no frame in the labels (alignment-invariant loss), the decoder reaches the standard accuracy after one global alignment (19.9° / 25.5° vs 20.2° / 25.4°): the circulant part carries the structure, the residual adds only the frame. A V1-trained decoder transfers to RL (frame gap 1.6°, residual correlation 0.77) but misplaces AL's frame (58° raw, 32° after rotation; residual correlation 0.31): the frame is consistent with the area's own anisotropy. Class-balanced populations (64 neurons per class, train and test) keep the frame (22.4° / 26.5° raw, identical after alignment), so the anchor is not the label density; a subtraction ablation of the class-level residual is invalid (subtracting a class-keyed offset with true classes leaks the classes, and even removing the whole class structure improves the score to 17°), so the carrier of the frame is located by exclusion and the cross-area pattern, not isolated. The dominant orientation of each area is also its most strongly co-correlated class (same-class mean correlation 0.071 vs 0.047–0.063 at 90° in twin V1, 0.095 vs 0.046–0.082 in vivo, 0.204 at 0° in AL), unchanged on class-balanced subsamples: the frame coincides with a correlation signature of the over-represented orientation that every Gram row carries, not with the counts. Cross-area transfer (V1-trained decoder on RL or AL) is seed-dependent and the 78° AL rotation of an earlier draft was a single-seed prior artefact; it is no longer used as evidence. Training on 140 neurons of one scan and testing on the others gives 38–44° (twin) and 37–39° (in vivo), the MICrONS analogue of Allen's single-animal failure. Synthetic test of the candidate anchor: a circulant model population with only like-tuned co-fluctuation peaked at 90° recovers the absolute frame (local-field model whose residual matches the cortical shape, corr 0.4–0.5 with in vivo V1: 15/15 seeds, 2–4° raw, at residual variance fractions 0.05–0.23 vs 0.16 twin / 0.30 in vivo; the earlier Fourier-field model also fixed it but its residual was anticorrelated with cortex), while sharper (κ up to 32) or more numerous cardinal neurons or stronger cardinal gain (up to 5x) never do (> 42° raw). An orientation-balanced loss does not recover obliques (twin: balanced error 25.3° vs 24.5°, oblique bins unchanged), so the cardinal readout is a limit of the relations, not a training shortcut. Queued: rotated-label training with an alignment-invariant loss (structure without frame), cross-area transfer, synthetic von Mises populations, bin-permuted activity decoders.
- **Synthetic populations (von Mises tuning, same pipeline).** With every anisotropy off the class-Gram is circulant and the decoder recovers the ordering (28–30° after one global alignment) but no frame (raw error above chance). A cardinal excess in the stimulus fixes the frame (9–15°); sharper cardinal tuning, stronger cardinal gain or an excess of cardinal-preferring neurons alone do not; count bias also degrades the structure. Details `outputs/synthetic.json`, script `synthetic.py`.
