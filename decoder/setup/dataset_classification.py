...
from decoder.underlying_datasets import DatasetClassificationDataModule
...
def setup_and_train_dataset_classification(seed, train_samples, valid_samples, project_name, config):
    ...
    wandb.init(
        ...
        config=config,
        name=f"dataset-classification-{decoder_class}-s{seed}",
        group=f"dataset-classification-{decoder_class}"
    )
